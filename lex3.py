import re
import copy
from create_name_table import name
import pickle
...
class Lex:
    ...
    def __init__(self):
        ...
        self.areanums = 1
        self.current_area = 0
        self.stack = []#store the area who is father
        self.if_new = True
            ...
            if string[0] == ' ' or string[0] == '\n' or string[0] == '\t':
                if state == 'state0':
                    ...
                    if tmp in self.keywords:
                        ...
                        ifexist = False
                        for index,n in enumerate(self.names):
                            if n.var_name == tmp and n.area == self.current_area:
                                ifexist = True
                                result.append(('var',index))
                                break
                        if not(ifexist):
                            n = name(var_name=tmp,area=self.current_area,father=self.stack[-1])
                            self.names.append(n)
                            #print n.area,n.father,n.var_name
                ...
                if state == 'state0':
                    if string[0] in self.symbols:#handle symbols
                        ...
                        if string[0] == '(' or string[0] == '{':
                            #self.stack.append(int(self.current_area))
                            #self.current_area = int(self.areanums)
                            if string[0] == '(':
                                self.if_new = True
                            if self.if_new:
                                self.stack.append(int(self.current_area))
                                ...
                                self.stack.append(int(self.current_area))
                                self.current_area = int(self.areanums-1)

                                
                            #print self.current_area,self.areanums,self.stack
                            ...
                            self.current_area = self.stack.pop()
                        ...
                        if tmp in self.keywords:
                            ...
                            for index,n in enumerate(self.names):
                                if n.var_name == tmp and n.area == self.current_area:
                                    ...
                                    break
                            if not(ifexist):
                                print tmp,self.current_area,self.stack[-1]
                                n = name(var_name=tmp,area=self.current_area,father=self.stack[-1])
                                self.names.append(n)
                                result.append(('var',len(self.names)-1))
